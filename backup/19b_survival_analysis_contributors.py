"""
19b_survival_analysis_contributors.py

Variante de 19_survival_analysis.py que adiciona contributors_count como
variável de controle no Cox e no RSF, para isolar o efeito de has_newcomer_labels
do tamanho da comunidade (confounding).

Hipótese: o HR alto de has_newcomer_labels (5.058 no script 19) é espúrio —
repos menores/mais novos adicionam essas labels justamente porque estão em risco.
Controlando por contributors_count o efeito deve diminuir.

Limitação reconhecida: contributors_count tem reverse-causality bias (repos que
sobrevivem mais acumulam mais contribuidores). Discutido no paper.

Outputs em: out/survival_analysis_contributors/ (separado do 19 original)
"""
...
OUT_DIR      = PROJECT_ROOT / "out" / "survival_analysis_contributors"
...
def load_data():
    ...
    # log1p versions for RSF (preprocessing applied here, not via Pipeline).
    ...
    df["raw_commits"]       = np.log1p(df["commits_per_month"])
    df["raw_contributors"]  = np.log1p(df["newcomers_per_month"])
    df["raw_forks"]         = np.log1p(df["forks_per_month"])
    df["raw_community_size"] = np.log1p(df["contributors_count"])
...
COX_BINARY_FEATURES = [
    ...
    "has_code_of_conduct_bin",
    "has_pr_template_bin",
    "has_issue_template_bin",
    "has_newcomer_labels_bin",
    "is_organization",
]
# Activity rates + community size to control for confounding on has_newcomer_labels.
# contributors_count acknowledges reverse-causality limitation (discussed in paper).
# stars_per_month excluded: high collinearity with forks (r=0.86).
# Pipeline applies log1p + StandardScaler internally.
...
COX_FEATURE_LABELS = {
    ...
    "contributors_count":      "Community Size (contributors)",
    "has_readme_bin":          "Has README",
    "has_contributing_bin":    "Has CONTRIBUTING",
    "has_code_of_conduct_bin": "Has Code of Conduct",
    "has_pr_template_bin":     "Has PR Template",
    ...
    # RSF raw aliases (log1p pre-applied, no Pipeline)
    "raw_commits":             "Commits/month",
    "raw_contributors":        "Newcomers/month",
    "raw_forks":               "Forks/month",
    "raw_community_size":      "Community Size (contributors)",
}
...
def fit_cox(df):
    ...
    ax.set_title(f"Cox PH — Feature Importance (test set)\n(C-index={c_index_test:.3f})")
...
def plot_hazard_ratios(df, c_index_test):
    ...
    Binary: HR for present vs absent.  Continuous: HR per 1-SD change (log1p scale)."""
...
def run_rsf(df):
    ...
    features = [f for f in (
        COX_BINARY_FEATURES + ["raw_commits", "raw_contributors", "raw_forks", "raw_community_size"]
    ) if f in df.columns]
